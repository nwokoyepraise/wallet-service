...
import { EmailAlreadyUsedException, VerifiedEmailAlreadyExistsException } from '../common/exceptions';
...
describe('UsersController', () => {
  ...
  describe('add-user', () => {
    ...
    it('should not add user because email exists but not verified', async () => {
      const addUserDto: AddUserDto = {
        user_id: 'USDJIJE99HJJO',
        email: 'email@email.com',
        password: 'hashed-password',
      ...
      expect(async ()=> {await usersController.addUser(addUserDto)}).rejects.toThrowError(EmailAlreadyUsedException())
    });
